...
def handle_task(mapped_dict, name, key, comments=None, post=None):
    """
...
class MapCreator:
    ...
    def create_dictionary(self):
        ...
        """
        for code in self.mapped_code:
            full_task_dict = {"class": "go.GraphLinksModel", "nodeDataArray": [], "linkDataArray": []}
            key = -1

            """ add the query task"""
            key, full_task_dict, query_key = self.create_query_task(code, full_task_dict, key)

            """ extract the class  """
            key, full_task_dict = self.create_class_task(code, full_task_dict, key, query_key)

            """ extract the implemented class  """
            key, ull_task_dict = self.add_implemented_task(code, full_task_dict, key)

            """extract the extended class """
            key, full_task_dict = self.add_extended_task(code, full_task_dict, key)
    ...
    def create_query_task(self, code, full_task_dict, key):
        ...
        code.set_key(key)
        query_key = key
        handle_task(mapped_task_dict, code.query, key, comments=None, post=code.text)
        key = key - 1
            ...
            for method in sub_class.Methods:
                for calling_method in method.calling_methods:
                    ...
                    if calling_method.get_key() == 0:
                        ...
                        current_key = calling_method.get_key()
